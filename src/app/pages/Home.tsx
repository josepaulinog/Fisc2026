...
// ---------------------------------------------------------------------------
// Four Moves — editorial magazine-spread teaser for the four-day arc. Sits
// between the About-FISC row and the delegate-room section. No cards, no
// buttons, no chrome — outsized tabular numerals + Instrument Serif italic
// theme words + body copy in a zigzag two-column layout. Anchors the page's
// editorial register at its centre.
//
// The four rows alternate indentation on desktop: 01 + 03 sit at the left
// edge of the content column, 02 + 04 step in by one grid track. The
// staggered offset reads as a typeset feature article, not a "4 cards
// horizontally" feature row.
// ---------------------------------------------------------------------------
...
const FOUR_MOVES = [
  {
    ...
    theme: "Frameworks.",
    body:
      "We open on the canonical questions. What is the budget for? How does a Chart of Accounts align to a National Development Plan? Who holds the line on outcomes once the money moves?",
  },
  {
    n: "02",
    day: "Tue · Jun 30",
    theme: "Practice.",
    body:
      "Workshop floors split by region — Africa, Americas, Caribbean, Asia-Pacific. Each group brings the hardest reform problem of the last twenty-four months. The room solves it together, country by country.",
  },
  {
    n: "03",
    day: "Wed · Jul 1",
    theme: "Roadmap.",
    body:
      "FreeBalance's product priorities are voted on by you. Twenty-three candidate features, one binding ballot. The features that ship next year carry your country's signature, not the vendor's guesswork.",
  },
  {
    n: "04",
    day: "Thu · Jul 2",
    theme: "Reflection.",
    body:
      "Cultural close, not a feature dump. Steel pan in the courtyard, dinner on the harbour, the soft handoff back to the work. You leave with peers, not just slides.",
  },
] as const;
...
function FourMoves() {
  return (
    <section
      ...
      style={{ backgroundColor: "#f6f4ef" }}
    >
      {/* Atmosphere — soft brand-orange radial in the top-right corner,
          fading into cream. Pure backdrop; no interaction. */}
      <div
        className="absolute -top-40 -right-40 w-[40rem] h-[40rem] rounded-full pointer-events-none opacity-[0.08] blur-3xl"
        style={{ backgroundColor: BRAND }}
      />
      <div
        className="absolute bottom-0 left-0 w-[30rem] h-[30rem] rounded-full pointer-events-none opacity-[0.04] blur-3xl"
        style={{ backgroundColor: BRAND }}
      />

      <div className="relative max-w-7xl mx-auto px-5 md:px-6">
        {/* Section header — restrained, lets the rows below carry the
            visual weight. Eyebrow + tight headline + one-line subhead. */}
        <motion.div
          variants={staggerSection}
          initial="hidden"
          whileInView="show"
        ...
        >
          ...
          <motion.h2
            variants={fadeUp}
            className="tracking-[-0.025em] text-neutral-950 mt-1"
            style={{ fontSize: TYPE.h2, lineHeight: 1.05, letterSpacing: TRACKING.snug }}
          >
            Four days, <GradientText>four moves.</GradientText>
          </motion.h2>
          <motion.p
            variants={fadeUp}
            className="mt-5 md:mt-6 text-neutral-700 max-w-xl"
            style={{ fontSize: "1.0625rem", lineHeight: 1.6 }}
          >
            FISC is structured like an argument, not a programme of talks.
            Each day moves the conversation one step closer to the country-by-country
            reform decisions you'll take home.
          </motion.p>
        </motion.div>

        {/* Rows — zigzag offset on desktop (md+). On mobile, all rows align
            left with no indentation; the visual variance comes from the
            row-by-row reveal stagger rather than the horizontal step.
            Each row is its own staggered subtree so the numeral, theme,
            and body arrive in waterfall order. */}
        <motion.div
          variants={staggerList}
          initial="hidden"
          whileInView="show"
          ...
          {FOUR_MOVES.map((m, i) => {
            ...
            return (
              <motion.article
                key={m.n}
                variants={fadeUp}
                className={`grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 py-8 md:py-14 border-t border-neutral-300/40 first:border-t-0 ${
                  indented ? "md:pl-[8%]" : ""
                }`}
              >
                {/* Numeral column — outsized tabular numeral with brand-orange
                    leading dot. Sits at top-left of the row; tight letter-
                    spacing and 200 weight read as editorial display, not as
                    a tag. */}
                <div className="md:col-span-3 flex md:flex-col items-baseline md:items-start gap-3 md:gap-4">
                  <span className="inline-flex items-center gap-2">
                    <span
                      className="w-1.5 h-1.5 rounded-full"
                      style={{ backgroundColor: BRAND }}
                  ...
                  <span
                    className="tabular-nums tracking-tight text-neutral-950 md:mt-1"
                    style={{
                ...
                {/* Theme + body column. The theme phrase is the Instrument
                    Serif italic moment — one word per day, the editorial
                    pin. Body sits below with a 60ch max-width so lines
                    break at the natural reading rhythm. */}
                <div className="md:col-span-9">
                  ...
                  >
                    <span
                      className="italic"
                      style={{
                        background: `linear-gradient(120deg, ${BRAND} 0%, #c64b00 100%)`,
                        WebkitBackgroundClip: "text",
                        WebkitTextFillColor: "transparent",
                        paddingInline: "0.05em",
                        marginInline: "-0.025em",
                      }}
                    >
                      {m.theme}
                    </span>
                  ...
                  <p
                    className="mt-4 md:mt-5 text-neutral-700 max-w-[60ch]"
        ...
        {/* Anchor link to the full agenda — single ghost CTA at the foot of
            the section so a reader who wants to commit can jump straight
            in. Aligned to the right column so it sits under the body
            paragraphs rather than orphaned at the page edge. */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={REVEAL_VIEWPORT}
          transition={{ duration: 0.4, delay: 0.1 }}
          className="mt-14 md:mt-20 md:pl-[8%] grid md:grid-cols-12 gap-6 md:gap-10"
        >
